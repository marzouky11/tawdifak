        ...
        <FormField control={form.control} name="programType" render={({ field }) => (
            <FormItem>
                <Select onValueChange={(value) => { field.onChange(value); if (value) form.setValue('customProgramType', ''); }} value={field.value} disabled={!!customProgramType}>
                    ...
                    <SelectContent>{programTypes.map(p => (<SelectItem key={p.value} value={p.value} className="text-right focus:bg-transparent focus:text-inherit data-[highlighted]:bg-transparent data-[highlighted]:text-inherit data-[state=checked]:bg-accent data-[state=checked]:text-accent-foreground">{p.label}</SelectItem>))}</SelectContent>
                </Select>
                <FormMessage />
            </FormItem>
        )} />
