...
export function PostJobForm({ categories, job, preselectedType }: PostJobFormProps) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      if (isEditing && job) {
        await updateAd(job.id, dataToSave, job.postType);
           ...
           <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col h-full">
            ...
            <div className="border-t bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 p-6 md:p-8">
                ...
                <div className="flex justify-between gap-4">
                    {isEditing ? (
                        <Button type="submit" disabled={isSubmitting} className="w-full mt-8 active:scale-95 transition-transform" size="lg" style={{backgroundColor: getThemeColor()}}>
