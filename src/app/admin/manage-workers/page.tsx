...
import { getWorkers, deleteAd } from "@/lib/data";
...
export default function AdminManageWorkersPage() {
  ...
  const fetchWorkers = useCallback(
    async (isLoadMore = false) => {
      ...
      try {
        ...
        setWorkers((prev) => (isLoadMore ? [...prev, ...res.data] : res.data));
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteAd(adToDelete, 'seeking_job');
      await revalidateAll();
      setWorkers((prev) => prev.filter((ad) => ad.id !== adToDelete));
  ...
  return (
    <>
      ...
      />
      {authLoading || (loading && workers.length === 0) ? (
        <FullPageLoader />
      ) : (
