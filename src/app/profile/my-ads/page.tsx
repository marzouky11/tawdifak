...
export default function MyAdsPage() {
  ...
  const [cursors, setCursors] = useState<{
    ...
    workers: FirestoreCursor;
    competitions: FirestoreCursor;
    immigration: FirestoreCursor;
    userAds: FirestoreCursor;
  }>({ jobs: null, workers: null, competitions: null, immigration: null, userAds: null });
  ...
  const [adToDelete, setAdToDelete] = useState<{
    ...
    postType?: "seeking_worker" | "seeking_job";
  } | null>(null);
  ...
  const fetchUserAds = useCallback(async (isLoadMore = false) => {
    if (!user) return;

    setLoading("userAds", true);
    try {
      const lastDoc = isLoadMore ? cursors.userAds : null;
      const res = await getUserAdsPaginated(user.uid, { limit: ITEMS_PER_PAGE, lastDoc });
      setJobRequests(prev => isLoadMore ? [...prev, ...res.data] : res.data);
      setCursors(prev => ({ ...prev, userAds: res.lastDoc }));
      setHasMore(prev => ({ ...prev, userAds: res.data.length === ITEMS_PER_PAGE }));
    } catch (error) {
      toast({ variant: "destructive", title: "فشل تحميل الإعلانات" });
    } finally {
      setLoading("userAds", false);
    }
  }, [user, cursors.userAds, toast]);

  useEffect(() => {
    if (user && !userData?.isAdmin && !authLoading && jobRequests.length === 0) {
      fetchUserAds();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user, userData, authLoading]);

  const fetchAdminData = useCallback(async (type: 'jobs' | 'workers' | 'competitions' | 'immigration', isLoadMore = false) => {
    ...
    try {
      ...
      if (type === 'jobs') {
        ...
        const res = await getWorkers({ limit: ITEMS_PER_PAGE, lastDoc });
        newData = res.data;
        newLastDoc = res.lastDoc;
  ...
  useEffect(() => {
    ...
    if (activeTab === 'workers' && jobRequests.length === 0) fetchAdminData('workers');
  ...
  const handleDelete = async () => {
    ...
    try {
      if (adToDelete.type === "ad") {
        await deleteAd(adToDelete.id, adToDelete.postType ?? 'seeking_job');
  ...
  const handleDeleteTrigger = (id: string, type: "ad" | "competition" | "immigration", postType?: "seeking_worker" | "seeking_job") => {
    setAdToDelete({ id, type, postType });
  };
        ...
        {userData?.isAdmin ? (
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full space-y-6">
            <TabsList className="grid w-full grid-cols-2 md:grid-cols-4 gap-2 h-auto">
              ...
              <TabsTrigger value="workers" className="data-[state=active]:bg-primary/10 data-[state=active]:text-primary data-[state=active]:font-bold data-[state=active]:shadow-sm">
                باحثون ({stats.workers})
              </TabsTrigger>
