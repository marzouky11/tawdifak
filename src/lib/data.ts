...
import type { FirestoreCursor, Job, Category, User, WorkType, Testimonial, Competition, Organizer, Article, Report, ContactMessage, ErrorLog, ImmigrationPost, PaginatedResponse } from './types';
...
async function getJobOffers(
  ...
  try {
    ...
    const adsRef = collection(db, 'jobs');
...
async function getWorkers(
  ...
  try {
    ...
    const adsRef = collection(db, 'workers');
...
// ملاحظة: تُستخدم هذه الدالة فقط للتحقق من إعلان "باحث عن عمل" الخاص
// بالمستخدم العادي (وليس المشرف)، لذلك تقرأ من مجموعة "workers" حصراً.
...
// ملاحظة: تُستخدم للمستخدم العادي في صفحة "إعلاناتي" فقط، وبما أن نشر عروض
// العمل أصبح حصراً للمشرفين، فإعلانات المستخدم العادي كلها من نوع "باحث عن
// عمل" — لذلك تقرأ من مجموعة "workers" حصراً.
export async function getUserAdsPaginated(
  userId: string,
  options: { limit?: number; lastDoc?: FirestoreCursor } = {}
): Promise<PaginatedResponse<Job>> {
  try {
    const { limit: pageLimit = 16, lastDoc } = options;
    const adsRef = collection(db, 'workers');
    ...
    const lastVisible = querySnapshot.docs[querySnapshot.docs.length - 1] || null;
    const jobs = mapSnapshotToJobs(querySnapshot);

    return { data: jobs, lastDoc: lastVisible };
  } catch (error) {
    console.error("Error fetching paginated user ads: ", error);
...
export async function getJobById(id: string): Promise<Job | null> {
  return withRetry(async () => {
    // بما أن هذه الدالة تُستدعى من صفحتي /jobs/[id] و /workers/[id] معاً بدون
    // معرفة مسبقة لنوع الإعلان، نجرّب مجموعة "jobs" أولاً ثم "workers".
    for (const collectionName of ['jobs', 'workers']) {
...
export async function postJob(jobData: Omit<Job, 'id' | 'createdAt' | 'likes' | 'rating' | 'postedAt' | 'isNew'>): Promise<{ id: string }> {
  try {
    if (jobData.postType === 'seeking_job' && jobData.userId) {
      const existingWorkerAdsQuery = query(
        collection(db, 'workers'),
        where('userId', '==', jobData.userId)
      );
      const existingWorkerAdsSnapshot = await getDocs(existingWorkerAdsQuery);
      if (!existingWorkerAdsSnapshot.empty) {
    ...
    // كل نوع إعلان يُحفظ في مجموعته الخاصة به: عروض العمل في "jobs"، وطلبات
    // العمل (باحث عن عمل) في "workers".
    const targetCollection = jobData.postType === 'seeking_worker' ? 'jobs' : 'workers';
    const newDocRef = doc(collection(db, targetCollection));
    const statsRef = doc(db, 'stats', 'general');

    await runTransaction(db, async (transaction) => {
      transaction.set(newDocRef, newJob);
      const counterField = jobData.postType === 'seeking_worker' ? 'jobs' : 'workers';
...
export async function updateAd(adId: string, adData: Partial<Job>, postType: 'seeking_worker' | 'seeking_job'): Promise<void> {
    try {
        const targetCollection = postType === 'seeking_worker' ? 'jobs' : 'workers';
...
export async function deleteAd(adId: string, postType: 'seeking_worker' | 'seeking_job'): Promise<void> {
  try {
    const targetCollection = postType === 'seeking_worker' ? 'jobs' : 'workers';
    const adRef = doc(db, targetCollection, adId);
    const statsRef = doc(db, 'stats', 'general');
    ...
    await runTransaction(db, async (transaction) => {
      ...
      const counterField = postType === 'seeking_worker' ? 'jobs' : 'workers';
...
export async function getUserTestimonial(userId: string): Promise<Testimonial | null> {
  try {
    ...
    const q = query(reviewsRef, where('userId', '==', userId), limit(1));
    const querySnapshot = await getDocs(q);
    ...
    console.error("Error fetching user's testimonial: ", error);
    return null;
  }
}

export async function updateTestimonial(testimonialId: string, testimonialData: { content: string; rating: number }): Promise<void> {
    try {
        const reviewRef = doc(db, 'reviews', testimonialId);
        await updateDoc(reviewRef, testimonialData);
    } catch (e) {
...
export async function addTestimonial(testimonialData: Omit<Testimonial, 'id' | 'createdAt' | 'postedAt'>): Promise<{ id: string }> {
    try {
        // يُسمح برأي واحد فقط لكل مستخدم — إن كان لديه رأي بالفعل، يجب
        // استخدام updateTestimonial بدلاً من إنشاء رأي جديد.
        ...
        if (!existingReviewSnapshot.empty) {
          throw new Error('DUPLICATE_TESTIMONIAL');
        }
        ...
        if (e instanceof Error && e.message === 'DUPLICATE_TESTIMONIAL') throw e;
...
export async function getCompetitions(
  ...
  try {
    ...
    if (searchQuery || location) {
      const q = query(competitionsRef, orderBy('createdAt', 'desc'), limit(200));
      const querySnapshot = await getDocs(q);
...
export async function getArticles(
  ...
  try {
    const { limit: pageLimit = 15, lastDoc } = options;
...
export async function getSavedAds(
  userId: string,
  options: { limit?: number; lastDoc?: FirestoreCursor } = {}
): Promise<PaginatedResponse<Job | Competition | ImmigrationPost>> {
  if (!userId) return { data: [], lastDoc: null };

  try {
    const { limit: pageLimit = 16, lastDoc } = options;
    const savedAdsRef = collection(db, 'users', userId, 'savedAds');

    const constraints: QueryConstraint[] = [orderBy('savedAt', 'desc')];
    if (lastDoc) {
      constraints.push(startAfter(lastDoc));
    }
    constraints.push(limit(pageLimit));

    const q = query(savedAdsRef, ...constraints);
    const snapshot = await getDocs(q);

    const nextCursor = snapshot.docs[snapshot.docs.length - 1] || null;

    if (snapshot.empty) return { data: [], lastDoc: null };

    // Preserve the order of this page's saved-ads docs (most recently saved first).
    ...
    snapshot.docs.forEach(doc => {
      ...
      typeById.set(doc.id, type);
    });

    const jobIds = orderedIds.filter(id => typeById.get(id) === 'job');
    const competitionIds = orderedIds.filter(id => typeById.get(id) === 'competition');
    const immigrationIds = orderedIds.filter(id => typeById.get(id) === 'immigration');

    const itemById = new Map<string, Job | Competition | ImmigrationPost>();
    ...
    const fetchBatch = async (collectionName: string, ids: string[], mapper: (doc: any) => any) => {
      ...
      snapshots.forEach(snap => {
        snap.docs.forEach(doc => {
          itemById.set(doc.id, mapper(doc));
        });
      });
    };
    ...
    await Promise.all([
      fetchBatch('jobs', jobIds, jobMapper),
      fetchBatch('workers', jobIds, jobMapper),
      fetchBatch('competitions', competitionIds, compMapper),
      fetchBatch('immigration', immigrationIds, immMapper),
    ]);

    // Reassemble in saved-order; skip any ids whose source ad/competition/post was deleted.
    const data = orderedIds
      .map(id => itemById.get(id))
      .filter((item): item is Job | Competition | ImmigrationPost => !!item);

    return { data, lastDoc: nextCursor };
  } catch (error) {
...
export async function getReports(
  ...
  try {
    const { lastDoc, limit: pageLimit = 16 } = options;
    ...
    return { data, totalCount: undefined, lastDoc: lastVisible };
  } catch (e) {
...
export async function addErrorLog(logData: { message: string; stack?: string; url?: string; digest?: string }): Promise<void> {
  try {
    ...
    // Intentionally swallow errors here: logging must never itself break the app.
    console.error("Error adding error log: ", e);
  }
}
...
export async function getErrorLogs(
  ...
  try {
    ...
    const logs = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as ErrorLog));

    return { data: logs, totalCount: undefined, lastDoc: lastVisible };
  } catch (e) {
...
export async function deleteErrorLog(logId: string): Promise<void> {
  try {
    ...
    console.error("Error deleting error log: ", e);
...
export async function getGlobalStats() {
  try {
    ...
    if (docSnap.exists()) {
      ...
      // ملاحظة مهمة: بعد إعادة تسمية الحقل من "seekers" إلى "workers"، أي
      // نشاط جديد (نشر/حذف باحث عن عمل) سيزيد قيمة الحقل الجديد "workers"
      // من الصفر، بينما يبقى الرصيد التاريخي محفوظاً في الحقل القديم
      // "seekers" في مستند "stats/general" الفعلي. نجمع الحقلين هنا مؤقتاً
      // لتفادي ظهور العداد أقل من قيمته الحقيقية. يُستحسن تحديث المستند
      // يدوياً مرة واحدة في Firebase Console (بجمع القيمتين في حقل واحد
      // "workers" وحذف الحقل القديم "seekers")، وعندها يمكن حذف هذا الجمع
      // المؤقت بأمان.
      return {
        jobs: data.jobs ?? 0,
        workers: (data.workers ?? 0) + (data.seekers ?? 0),
        competitions: data.competitions ?? 0,
        immigration: data.immigration ?? 0,
        users: data.users ?? 0,
      };
    }
    return { jobs: 0, workers: 0, competitions: 0, immigration: 0, users: 0 };
  } catch (error) {
...
export const getCachedInitialWorkers = unstable_cache(
  async () => {
    const { data } = await getWorkers({ limit: 16 });
    return data;
  },
  ['initial-workers-list'],
  { tags: ['workers-list', 'workers-home'], revalidate: 3600 }
);

export const getCachedInitialTestimonials = unstable_cache(
  async () => {
    const { data } = await getTestimonials({ limit: 16 });
    return data;
  },
...
export const getCachedInitialArticles = unstable_cache(
  async () => {
    const { data } = await getArticles({ limit: 15 });
    return data;
  },
...
export const getCachedHomePageCompetitions = unstable_cache(
  async () => {
    return getCompetitions({ count: 4 });
  },
  ['home-competitions'],
...
export const getCachedHomePageImmigration = unstable_cache(
  async () => {
    return getImmigrationPosts({ count: 8 });
  },
  ['home-immigration'],
  { tags: ['immigration-home'], revalidate: 3600 }
);

export const getCachedHomePageWorkers = unstable_cache(
  async () => {
    return getWorkers({ count: 4 });
  },
  ['home-workers'],
  { tags: ['workers-home'], revalidate: 3600 }
);
...
export { getJobOffers, getWorkers };
